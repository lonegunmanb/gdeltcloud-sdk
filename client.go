...
func (c *Client) get(ctx context.Context, path string, query url.Values, out any) error {
	...
	if err != nil {
		return fmt.Errorf("gdeltcloud: build request: %w", err)
	}
	...
	if err != nil {
		return fmt.Errorf("gdeltcloud: request failed: %w", err)
	}
	defer resp.Body.Close()

	body, err := io.ReadAll(resp.Body)
	if err != nil {
		return fmt.Errorf("gdeltcloud: read response: %w", err)
	}
	...
	if jsonErr := json.Unmarshal(body, &env); jsonErr != nil {
		...
		return fmt.Errorf("gdeltcloud: decode response: %w", jsonErr)
	}

	if resp.StatusCode < 200 || resp.StatusCode >= 300 {
		return &APIError{StatusCode: resp.StatusCode, Message: env.Error, Body: string(body)}
	}
